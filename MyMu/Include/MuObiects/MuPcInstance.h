...
public:
	unsigned long getCharIdInDb()
	{
		return p_CharIdInDb;
	}

	void setCharIdInDb(unsigned long id)
	{
		p_CharIdInDb = id;
	}
	...
	void IUpdateCurMpSt()
	{
		SUpdateManaStamina*t = new SUpdateManaStamina(getStatCurMp(),
				getStatCurSt());
		Send(t);
		delete t;
	}
...
public:
	...
	void StatCalclMaxMp()
	{
		...
		case 0x20:
		case 0x30:
			max = (int) (20 + (lvl * 0.5) + (enr * 1));
			break;
		case 0x40:
		case 0x50:
			max = (int) (30 + (lvl * 1.5) + (enr * 1.5));
			break;
		case 0x60:
			max = 60 + (lvl * 1) + (enr * 2);
			break;
