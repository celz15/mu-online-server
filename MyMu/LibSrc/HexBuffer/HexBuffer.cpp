...
int HexBuff::readI(bool HiLo)
	{
	int i;
	if(HiLo) i=buf[pos]*256+buf[pos+1];
	else i=buf[pos+1]*256+buf[pos];
	pos+=2;
	return i;
	};	
...
std::string HexBuff::_uchar2str(unsigned char h,std::string przed){
       ...
       sprintf(t, "%x",char(h));
       std::string hex=t;
       if (hex.size()>5) hex.erase(0,hex.size()-2);
       if (hex.size()==1)hex=przed+"0"+hex;
       if (hex.size()==2)hex=przed+hex;
       return hex;
       };
...
unsigned char HexBuff::getType1Pack()
{
  ...
  case 0xc1:
  case 0xc3: return buf[2];break;
  case 0xc2:
  case 0xc4: return buf[3];break;
  };
};
unsigned char HexBuff::getType2Pack(){
  ...
  case 0xc1:
  case 0xc3:return buf[3];break;
  case 0xc2:
  case 0xc4:return buf[4];break;
  };
};
