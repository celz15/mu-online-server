  ...
  if(_allSpotV.size()==0)
      ...
      for(int a=0;a<_allSpotV.size();a++)
	{
	  ...
	  for(int c=0;c<=s->getC()-1;c++)
		...
		newMuMonsterInstance(t,(unsigned char)s->startX(),(unsigned char)s->startY(),this->getCode());
	      //	
	      storeNewObiect(mob);
	      //mob->RunMoveInst();
	    };
...
void MuMap::updatePortView(MuViewPortSet * view)
{
  ...
  if ( !_allObiectV.empty())
    for (std::vector<unsigned short>::iterator  i =_allObiectV.begin() ; i != _allObiectV.end();i++)
      {
	if((*i) == view->getIndex()) continue;
	MuViewPortSet * his=ObiectPool::getInstance()->getObject(*i)->getViewPort();
	his->addViewPort(ObiectPool::getInstance()->getDistance(his->getIndex(),view->getIndex(),his->getViewRad()));
	view->addViewPort(ObiectPool::getInstance()->getDistance(view->getIndex(),(*i),view->getViewRad()));	
      };		
};
