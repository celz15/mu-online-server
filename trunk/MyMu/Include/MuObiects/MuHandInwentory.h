...
public:
  ...
  bool putItemInInwentory(ItemInInwentory *it)
  {
    ...
    if(inw[it->inw_slot]!=NULL) return false;
    inw[it->inw_slot]=it;
    if(it->inw_slot >12)
      updateItemMap(it->inw_ItemInfo->i_InfoHight,
		    it->inw_ItemInfo->i_InfoLenght,
		    getXFromArrayPos(it->inw_slot),
		    getYFromArrayPos(it->inw_slot));
    return true;
  };
  ...
  std::vector<ItemInInwentory *> getAsVector()
  {
    ...
    for (int i =0 ; i<=75;i++)
      if(inw[i]!=NULL)list.push_back(inw[i]);
    return list;
  };
    
  void updateItemMap(unsigned char h,unsigned char l, unsigned char x , unsigned char y)
  {
    unsigned char tmap[h-1];
    for (int i =0 ; i<h;i++)
      tmap[i]=0xffff<<8-l;
  ...
  unsigned char checkOnPos(unsigned char x, unsigned char y,unsigned char h,unsigned char *tmap)
  {
    unsigned char ret =0;
    for (int i = 0; i<h;i++)
