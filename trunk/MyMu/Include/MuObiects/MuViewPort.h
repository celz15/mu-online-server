...
public:
  ...
  {
    if (i >getViewSize() ) 
      printf("Error try to acces to offindex of array in [FIle:%s,Line:%d]\n",__FILE__,__LINE__);
    return _vPort[i];
  };
  ...
  void checkViewPort()
  {
	...
	if(_vPort[i].c_State!=MuViewPort::S_Empty)
	  {
	    _vPort[i]= ObiectPool::getInstance()->getDistance(_vIndex,_vPort[i].o_Index,getViewRad());
	    if(_vPort[i].o_dist>_vRad)_vPort[i].c_State=MuViewPort::S_ToForget;
	  };
      };
  };
  ...
  unsigned char searchId(unsigned short id)
  {
    for (int i = 0 ; i < getViewSize();i++)
      {
	if (_vPort[i].o_Index==id) return i;
      };
    return 0xff;
  };
  ...
  void printMe()
  {
    printf("\nPointView[Size:%d] for [Index:%d]in [Rad:%d]\n",_vSize,_vIndex,_vRad);
    for (int i =0 ; i<getViewSize();i++)
      {
	if(_vPort[i].c_State!=MuViewPort::S_Empty)_vPort[i].PrintMe();
      };
  ...
  void addViewPort(MuViewPort p)
  {
	...
	if (temp.c_State==MuViewPort::S_New)
      ...
      {
	if((temp.c_State==MuViewPort::S_New)&&(_vPort[pos].c_State==MuViewPort::S_Known))  
	  _vPort[pos].o_dist= temp.o_dist;
	else	
	  _vPort[pos]=temp; //update dest and forget flag
      };
    delete &temp;
    //  printMe();
  };
